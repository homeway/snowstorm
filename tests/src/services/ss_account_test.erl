...
to_test() ->
    ...
    Res = {res, account},
    W:reg_server(Res, ss_account, [#{res=>ss_account2}]),
    ?assertEqual(true, is_pid(W:find(Res))),

    %% service server: {service, offline}
    W:reg_server(?offline, ss_offline, [#{res=>ss_offline}]),
    ...
    W:call(Res, [find, Id2]),
    W:reg_server(Adi, ss_account, [#{res=>ss_account2}]),
    ...
    %% 出席
    W:reg_server(Yifan, ss_account, [#{res=>ss_account2}]),
    ...
    W:reg_server(Homeway, ss_account, [#{res=>ss_account2}]),
    ...
    %% wenxuan登录后
    W:reg_server(Wenxuan, ss_account, [#{res=>ss_account2}]),
    ...
    W:reg_server(Xiaojie, ss_account, [#{res=>ss_account2}]),
    ...
    W:reg_server(Zhuhao, ss_account, [#{res=>ss_account2}]),
